...
registerMooseObject("TestTrussApp", PlasticTruss2);

defineLegacyParams(PlasticTruss2);

InputParameters
PlasticTruss2::validParams()
{
  InputParameters params = LinearElasticTruss2::validParams();
...
PlasticTruss2::PlasticTruss2(const InputParameters & parameters)
  : LinearElasticTruss2(parameters),
    ...
    _max_its(1000)
{
  if (!parameters.isParamSetByUser("hardening_constant") && !isParamValid("hardening_function"))
    mooseError("PlasticTruss2: Either hardening_constant or hardening_function must be defined");

  if (parameters.isParamSetByUser("hardening_constant") && isParamValid("hardening_function"))
    mooseError("PlasticTruss2: Only the hardening_constant or only the hardening_function can be "
               "defined but not both");
}

void
PlasticTruss2::initQpStatefulProperties()
{
...
void
PlasticTruss2::computeQpStrain()
{
  _total_stretch[_qp] = _current_length / _origin_length - 1.0;
}

void
PlasticTruss2::computeQpStress()
{
  ...
  if (yield_condition > 0.0)
  {
           ...
           std::abs(residual / reference_residual) > _relative_tolerance)
    {
      ...
      if (iteration > _max_its) // not converging
        throw MooseException("PlasticTruss2: Plasticity model did not converge");
    }
...
Real
PlasticTruss2::computeHardeningValue(Real scalar)
{
...
Real PlasticTruss2::computeHardeningDerivative(Real /*scalar*/)
{
  if (_hardening_function)
  {
    const Real strain_old = _plastic_strain_old[_qp];
