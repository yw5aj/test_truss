...
#include "StressDivergenceTensorsSpring.h"
...
registerMooseObject("TestTrussApp", StressDivergenceTensorsSpring);

defineLegacyParams(StressDivergenceTensorsSpring);

InputParameters
StressDivergenceTensorsSpring::validParams()
{
...
StressDivergenceTensorsSpring::StressDivergenceTensorsSpring(const InputParameters & parameters)
  : Kernel(parameters),
...
void
StressDivergenceTensorsSpring::initialSetup()
{
  _orientation = &_subproblem.assembly(_tid).getFE(FEType(), 1)->get_dxyzdxi();
}

void
StressDivergenceTensorsSpring::computeResidual()
{
...
Real
StressDivergenceTensorsSpring::computeStiffness(unsigned int i, unsigned int j)
{
  RealGradient orientation((*_orientation)[0]);
  orientation /= orientation.norm();
...
void
StressDivergenceTensorsSpring::computeJacobian()
{
...
void
StressDivergenceTensorsSpring::computeOffDiagJacobian(MooseVariableFEBase & jvar)
{
